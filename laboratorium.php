<?php
// laboratorium.php – Detail Laboratorium dengan Tabulator JS | AdminLTE 4
...
<head>
  ...
  <title>Detail Laboratorium | Kasir Laporan Keuangan</title>
  <meta name="description" content="Laporan detail pendapatan Laboratorium kasir pendaftaran.">
  ...
  <main class="app-main">
    <div class="app-content-header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-6">
            <h3 class="mb-0 fw-bold"><i class="fas fa-flask text-warning me-2"></i>Detail Total Laboratorium</h3>
          </div>
      ...
      <div class="container-fluid">
          ...
          <div class="col-md-4 col-sm-6">
            <div class="card shadow-sm stat-card total-card p-3">
              <div class="d-flex align-items-center gap-3">
                ...
                <div>
                  <div class="text-muted small fw-semibold">Total Pendapatan Lab</div>
                  <div class="fs-5 fw-bold text-warning" id="statTotal">Rp 0</div>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4 col-sm-6">
            <div class="card shadow-sm stat-card item-card p-3">
              <div class="d-flex align-items-center gap-3">
                ...
                <div>
                  <div class="text-muted small fw-semibold">Total Item Pemeriksaan</div>
                  <div class="fs-5 fw-bold text-primary" id="statItems">0 Item</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="card shadow-sm stat-card avg-card p-3">
              <div class="d-flex align-items-center gap-3">
                ...
                <div>
                  ...
                  <div class="fs-5 fw-bold text-success" id="statAvg">Rp 0</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="card shadow-sm">
          <div class="card-header d-flex align-items-center justify-content-between py-3">
            <h5 class="card-title mb-0 fw-bold"><i class="bi bi-table text-primary me-2"></i>Rincian Pendapatan Laboratorium</h5>
            <span class="badge bg-secondary no-print" id="badgePeriode">Semua Data</span>
          </div>
          <div class="card-body p-3">
            <div class="print-header px-4 pt-3">
              <h2>Laporan Pendapatan Laboratorium</h2>
              <p id="printPeriode">Periode: Semua Data</p>
              <p>Dicetak: <?= date('d/m/Y H:i') ?></p>
            </div>

            <!-- Tabulator Table (Screen) -->
            <div id="labTable" class="no-print"></div>

            <!-- Summary Screen -->
            <div class="mt-3 p-3 bg-light rounded border d-flex justify-content-between align-items-center no-print">
              <span class="fw-bold text-dark"><i class="bi bi-sigma me-1"></i>JUMLAH TOTAL: <span class="text-warning fs-5" id="sumTotalText">Rp 0</span></span>
            </div>
            ...
            <table class="table table-bordered align-middle mb-0 print-only-table" id="tabelPrint">
              <thead>
                <tr class="text-center">
                  ...
                  <th class="text-center" style="width:140px">Jumlah Item</th>
                  <th class="text-end" style="width:220px">Total Laboratorium</th>
                </tr>
              </thead>
...
function initTabulator() {
  tabulatorTable = new Tabulator('#labTable', {
    data: [],
    layout: 'fitColumns',
    ...
    columns: [
      ...
      { title: 'Jumlah Item Lab', field: 'item_count', headerHozAlign: 'center', hozAlign: 'center', formatter: cell => `${cell.getValue()} Item` },
      { title: 'Total Laboratorium', field: 'total_lab', headerHozAlign: 'right', hozAlign: 'right', formatter: cell => `<span class="fw-bold text-warning">${fmt(cell.getValue())}</span>` }
    ]
  });
}
...
async function loadData(start = '', end = '') {
  ...
  try {
    ...
    const totalItems = json.total_items || 0;
    const count      = json.count || 0;
    const avg        = count > 0 ? total / count : 0;

    document.getElementById('statTotal').textContent   = fmt(total);
    document.getElementById('statItems').textContent   = totalItems + ' Item';
    ...
    tbody.innerHTML = rows.map((r, idx) => `
      <tr>
        ...
        <td style="border:1px solid #000; padding:6px; text-align:center; color:#000;">${r.item_count} Item</td>
        <td style="border:1px solid #000; padding:6px; text-align:right; font-weight:bold; color:#000;">${fmt(r.total_lab)}</td>
      </tr>
    `).join('');
    ...
    tfoot.innerHTML = `
      <tr style="border-top:2.5px solid #000;">
        ...
        <td style="border:1px solid #000; padding:6px; text-align:center; font-weight:bold; color:#000;">${totalItems} Item</td>
  ...
  document.getElementById('btnCetak').addEventListener('click', () => {
    ...
    printView.innerHTML = `
      <div style="text-align:center; border-bottom:2px solid #000; padding-bottom:8px; margin-bottom:14px;">
        <h1 style="font-size:15pt; font-weight:800; text-transform:uppercase; margin:0 0 4px 0; color:#000;">Laporan Pendapatan Laboratorium</h1>
      ...
      <table style="width:100%; border-collapse:collapse; font-size:9.5pt; font-family:Arial, sans-serif; border:1.5px solid #000;">
        <thead>
          <tr style="border-bottom:2.5px solid #000; background:#f0f0f0;">
            ...
            <th style="border:1px solid #000; padding:6px; text-align:center; color:#000;">Jumlah Item Lab</th>
            <th style="border:1px solid #000; padding:6px; text-align:right; width:220px; color:#000;">Total Laboratorium</th>
          </tr>
        </thead>
        <tbody>
          ${tbodyHTML}
