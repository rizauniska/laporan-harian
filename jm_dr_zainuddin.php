...
<head>
  ...
  <style>
    ...
    #zainuddinTable { border-radius: 6px; overflow: hidden; font-size: .875rem; }
    ...
    .stat-card.total-card { border-color: #0dcaf0; }
    ...
    @media print {
      ...
      .app-header, .app-sidebar, .app-footer, .no-print, .filter-area, .stat-cards, .card-tools, .breadcrumb, .app-content-header, .tabulator, #zainuddinTable { display: none !important; }
    ...
    <div class="sidebar-wrapper">
      <nav class="mt-2">
        <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu">
          ...
          <li class="nav-item"><a href="jm_dr_zainuddin.php" class="nav-link active"><i class="nav-icon fas fa-user-md"></i><p>JM dr. Zainuddin</p></a></li>
          ...
          <div class="col-md-4 col-sm-6">
            <div class="card shadow-sm stat-card total-card p-3">
              <div class="d-flex align-items-center gap-3">
                <div style="width:48px;height:48px;border-radius:10px;background:rgba(13,202,240,.12);display:flex;align-items:center;justify-content:center;font-size:1.4rem;color:#0dcaf0">
...
function initTabulator() {
  tabulatorTable = new Tabulator('#zainuddinTable', {
    ...
    movableColumns: true,
    placeholder: 'Tidak ada data JM dr. Zainuddin',
    columns: [
      { title: 'No', formatter: 'rownum', width: 65, headerHozAlign: 'center', hozAlign: 'center', headerSort: false },
      {
        ...
        formatter: cell => `<span class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 px-2 py-1 badge-date"><i class="bi bi-calendar3 me-1"></i>${fmtTglIndo(cell.getValue())}</span>`
      },
      {
        title: 'JM dr. Zainuddin', field: 'jm_dr_zainuddin', headerHozAlign: 'right', hozAlign: 'right',
  ...
  try {
    ...
    tbody.innerHTML = rows.map((r, idx) => `
      <tr>
        ...
        <td class="text-end fw-bold text-info">${fmt(r.jm_dr_zainuddin)}</td>
      </tr>
    `).join('');
